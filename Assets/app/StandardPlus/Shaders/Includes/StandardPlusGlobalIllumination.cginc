...
#ifndef STANDARD_PLUS_GLOBAL_ILLUMINATION_INCLUDED //Standard Plus Code
#define STANDARD_PLUS_GLOBAL_ILLUMINATION_INCLUDED //Standard Plus Code
...
#include "StandardPlusImageBasedLighting.cginc" //Standard Plus Code

#include "UnityStandardUtils.cginc"

#include "StandardPlusShadowLibrary.cginc" //Standard Plus Code
...
inline UnityGI UnityGI_Base(UnityGIInput data, half occlusion, half3 normalWorld)
{
    ...
    //Standard Plus Code
    	...
    	half4 bakedColorTex = tex2D(data.lightTex, data.lightmapUV.xy) * half4(data.lightTint, 1);
    	half4 bakedColor = bakedColorTex;

    	#if defined(_CUSTOM_LIGHTMAP2)
    		half4 bakedColorTex2 = tex2D(data.lightTex2, data.lightmapUV.xy) * half4(data.lightTint2, 1);
    		bakedColor += bakedColorTex2;
    	#endif

    	#if defined(_CUSTOM_LIGHTMAP3)
    		half4 bakedColorTex3 = tex2D(data.lightTex3, data.lightmapUV.xy) * half4(data.lightTint3, 1);
    		bakedColor += bakedColorTex3;
    	#endif

    	#if defined(_CUSTOM_LIGHTMAP4)
    		half4 bakedColorTex4 = tex2D(data.lightTex4, data.lightmapUV.xy) * half4(data.lightTint4, 1);
    		bakedColor += bakedColorTex4;
    	#endif

    	#if defined(_CUSTOM_LIGHTMAP5)
    		half4 bakedColorTex5 = tex2D(data.lightTex5, data.lightmapUV.xy) * half4(data.lightTint5, 1);
    		bakedColor += bakedColorTex5;
    	#endif
    	
    	o_gi.indirect.diffuse = bakedColor.rgb;

        #if defined(LIGHTMAP_SHADOW_MIXING) && !defined(SHADOWS_SHADOWMASK) && defined(SHADOWS_SCREEN)
            ResetUnityLight(o_gi.light);
            o_gi.indirect.diffuse = SubtractMainLightWithRealtimeAttenuationFromLightmap(o_gi.indirect.diffuse, data.atten, bakedColor, normalWorld);
        #endif
