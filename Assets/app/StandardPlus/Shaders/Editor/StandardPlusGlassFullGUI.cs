...
public class StandardPlusGlassFullGUI : ShaderGUI {
	...
	private static class Styles
	{
		...
		public static GUIContent thicknessMapText = new GUIContent ("Thickness Map", "To create this map, flip your model's normals and bake an ambient occlusion map.");
		public static GUIContent affectsRefractionText = new GUIContent("Map Affects Refraction", "Toggles whether the thickness map affects the refraction amount");
		public static GUIContent affectsColorText = new GUIContent("Map Affects Color", "Toggles whether the thickness map affects the color");
		public static GUIContent refractionText = new GUIContent ("Refraction Controls", "");
		public static GUIContent refractionAmountText = new GUIContent ("Refraction Amount", "");
		public static GUIContent colorLevelsText = new GUIContent ("Color Levels", "");
		public static GUIContent refractionLevelsText = new GUIContent ("Refraction Levels", "");
		...
		public static GUIContent blurText = new GUIContent ("Enable Blur", "Enables refraction blur.");
		public static GUIContent sizeBlurText = new GUIContent ("Blur Amount", "");
		public static GUIContent fresnelLengthText = new GUIContent ("Fresnel Length", "");
		public static GUIContent fresnelAmountText = new GUIContent ("Fresnel Amount", "");
		public static string plusRefracText = "Plus Glass";
	...
	MaterialProperty affectsRefraction = null;
	MaterialProperty affectsColor = null;
	MaterialProperty refractionAmount = null;
	...
	MaterialProperty inMinCOLOR = null;
	MaterialProperty inMiddleCOLOR = null;
	MaterialProperty inMaxCOLOR = null;
	MaterialProperty outMinCOLOR = null;
	MaterialProperty outMaxCOLOR = null;
	MaterialProperty sizeBlur = null;
	MaterialProperty fresnelLength = null;
	MaterialProperty fresnelAmount = null;
	//End of Standard Plus Code
	...
	bool colorLevels = false;
	bool refractionLevels = false;
	bool blurOn = false;
	...
	public void FindProperties(MaterialProperty[] props)
	{
		...
		affectsRefraction = FindProperty("_AffectsRefraction", props, false);
		affectsColor = FindProperty("_AffectsColor", props, false);
		refractionAmount = FindProperty ("_RefractionAmount", props);
		...
		inMinCOLOR = FindProperty ("_INMinCOLOR", props);
		inMiddleCOLOR = FindProperty ("_INMiddleCOLOR", props);
		inMaxCOLOR = FindProperty ("_INMaxCOLOR", props);
		outMinCOLOR = FindProperty ("_OUTMinCOLOR", props);
		outMaxCOLOR = FindProperty ("_OUTMaxCOLOR", props);
		sizeBlur = FindProperty ("_SizeBlur", props);
		fresnelLength = FindProperty ("_FresnelLength", props);
		fresnelAmount = FindProperty ("_FresnelAmount", props);
		//End of Standard Plus Code
	}
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
	{
		...
		if (m_FirstTimeApply)
		{
			...
			//Standard Plus Code
			SetKeyword("_REFRACTION", true);
			SetKeyword("_FULLBACKGROUND", true);
	...
	public void ShaderPropertiesGUI(Material material)
	{
		...
		EditorGUI.BeginChangeCheck();
		{
			...
			//Standard Plus Code
			EditorGUILayout.Space();
			GUILayout.Label(Styles.plusRefracText, EditorStyles.boldLabel);
			DoRefractionArea ();
			EditorGUILayout.Space();
...
//		}
		...
		BlendMode blendMode = BlendMode.Transparent;
	...
	void DoAlbedoArea(Material material)
	{
		...
		Color colorAlbedo = albedoColor.colorValue;

		if (albedoColor.textureValue == null && albedoChanged && mapChanged) {
			albedoColor.colorValue = new Color(albedoColor.colorValue.r, albedoColor.colorValue.g, albedoColor.colorValue.b, lastAlpha);
			albedoChanged = false;
			mapChanged = false;
		}

		if(albedoMap.textureValue != null && !mapChanged){
			lastAlpha = albedoColor.colorValue.a;
			albedoChanged = true;
			mapChanged = true;
			colorAlbedo.a = 1;
			albedoColor.colorValue = colorAlbedo;
		}
	...
	void DoRefractionArea(){
		...
		m_MaterialEditor.ShaderProperty (refractionAmount, Styles.refractionAmountText);
		m_MaterialEditor.TexturePropertySingleLine(Styles.thicknessMapText, thicknessMap);

		if (affectsRefraction != null)
			m_MaterialEditor.ShaderProperty(affectsRefraction, Styles.affectsRefractionText);
		if (affectsColor != null)
			m_MaterialEditor.ShaderProperty(affectsColor, Styles.affectsColorText);

		EditorGUI.BeginChangeCheck();
		refractionLevels = EditorGUILayout.Toggle(Styles.refractionLevelsText,
			IsKeywordEnabled(m_MaterialEditor.target as Material, "_REFRACTIONLEVELS"));
		if (EditorGUI.EndChangeCheck()) {
			SetKeyword("_REFRACTIONLEVELS", refractionLevels);
		}
		...
		EditorGUI.BeginChangeCheck();
		colorLevels = EditorGUILayout.Toggle(Styles.colorLevelsText,
			IsKeywordEnabled(m_MaterialEditor.target as Material, "_COLORLEVELS"));
		if (EditorGUI.EndChangeCheck()) {
			SetKeyword("_COLORLEVELS", colorLevels);
		}

		if(colorLevels){
			EditorGUI.indentLevel += 2;
			m_MaterialEditor.ShaderProperty (inMinCOLOR, Styles.inMinText);
			m_MaterialEditor.ShaderProperty (inMiddleCOLOR, Styles.inMiddleText);
			m_MaterialEditor.ShaderProperty (inMaxCOLOR, Styles.inMaxText);
			m_MaterialEditor.ShaderProperty (outMinCOLOR, Styles.outMinText);
			m_MaterialEditor.ShaderProperty (outMaxCOLOR, Styles.outMaxText);
			EditorGUI.indentLevel -= 2;
		}

		m_MaterialEditor.ShaderProperty (fresnelAmount, Styles.fresnelAmountText);
		m_MaterialEditor.ShaderProperty (fresnelLength, Styles.fresnelLengthText);

		EditorGUI.BeginChangeCheck();
		blurOn = EditorGUILayout.Toggle(Styles.blurText,
			IsKeywordEnabled(m_MaterialEditor.target as Material, "_BLUR_ON"));
		if (EditorGUI.EndChangeCheck()) {
			SetKeyword("_BLUR_ON", blurOn);
		}

		if(blurOn)
			m_MaterialEditor.ShaderProperty (sizeBlur, Styles.sizeBlurText);
